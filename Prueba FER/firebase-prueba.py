...
firebase = firebase.firebase.FirebaseApplication('https://soatp-2dfc9.firebaseio.com', authentication=None)

def log_puertas(response):
    with open('%TEMP%/%s.json' % response.keys()[0], 'w') as users_file:
        users_file.write(json.dumps(response, cls=firebase.jsonutil.JSONEncoder))

firebase.get_async('/baseDatosSoa/puertas/Puerta Fer', None, {'print': 'pretty'}, callback=log_puertas)
