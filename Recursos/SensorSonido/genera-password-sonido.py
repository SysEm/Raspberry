...
GPIO.setup(micro, GPIO.IN, pull_up_down=GPIO.PUD_UP) ## GPIO 25 como entrada
GPIO.setup(ledRojo, GPIO.OUT) ## GPIO 16 como salida
GPIO.setup(ledVerde, GPIO.OUT) ## GPIO 20 como salida
...
def escribirArchivo(valor):
    arch=open('./password.txt','w+')
    arch.write(valor)
    arch.close()
    return None
